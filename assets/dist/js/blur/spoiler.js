...
(function ($) {
  ...
  $.fn.spoilerAlert = function (opts) {
    ...
    return this.each(function () {
      ...
      var applyBlur = function applyBlur(radius) {
        currentBlur = radius;
        if (browser.msie) {
          ...
          $spoiler.css('filter', filterValue).css('-webkit-filter', filterValue).css('-moz-filter', filterValue).css('-o-filter', filterValue).css('-ms-filter', filterValue);
        }
      };

      var performBlur = function performBlur(targetBlur, direction) {
        cancelTimer();
      ...
      var ieLessThanTen = function ieLessThanTen() {
      ...
      $spoiler.on('mouseover', function (e) {
        $spoiler.css('cursor', 'pointer').attr('title', hintText);
      ...
      $spoiler.on('click', function (e) {
        if ($spoiler.data('spoiler-state') == 'shrouded') {
          $spoiler.data('spoiler-state', 'revealed').attr('title', '').css('cursor', 'auto');
          performBlur(0, -1);
        } else {
          $spoiler.data('spoiler-state', 'shrouded').attr('title', hintText).css('cursor', 'pointer');
