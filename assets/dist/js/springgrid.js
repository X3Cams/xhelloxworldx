...
var {
	...
	makeResponsive,
	measureItems,
	layout as this.layouts,
		enterExitStyle as this.enterExitStyles
} = require('../../../src/index');
...
export default class extends Component {
	...
	render() {
		const {
			children, useCSS, responsive, layout, enterExitStyle,
			duration, easing, stiffness, damping, gutters, columns, ...rest
		} = this.props;
		...
		return ( < Grid {...rest
			...
			enter = {
				gridEnterExitStyle.enter
			}
			entered = {
				gridEnterExitStyle.entered
			}
			exit = {
				gridEnterExitStyle.exit
			}
			perspective = {
				600
			}
			duration = {
				useCSS ? duration : null
			}
			easing = {
				useCSS ? easing : null
			}
			springConfig = {
				!useCSS && stiffness && damping ? {
					stiffness, damping
				} : null
			} > {
				children
